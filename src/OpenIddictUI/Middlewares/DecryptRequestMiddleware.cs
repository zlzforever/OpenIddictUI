...
namespace OpenIddictUI.Middlewares;
...
public class DecryptRequestMiddleware(RequestDelegate next)
{
        ...
        ILogger<DecryptRequestMiddleware> logger)
    {
        ...
        if (HttpMethods.All(x => !x.Equals(context.Request.Method, StringComparison.OrdinalIgnoreCase)))
        {
            await next(context);
            return;
        }
        ...
        {
            var forceEncryptBody = context.Request.Path.Value != null &&
                                   serviceOptions.Value.ForceEncryptPaths.Any(x =>
                                       context.Request.Path.Value.Equals(x, StringComparison.OrdinalIgnoreCase));
            if (forceEncryptBody)
            {
    ...
    private static async Task DecryptV1Body(HttpContext context, string encryptKey)
    {
        ...
        if (!string.IsNullOrEmpty(encryptedBody))
        {
            ...
            using var aes = CreateAesCbc(key, parts[0]);
            var ciphertext = parts[1];
            // 解密请求body
            var decryptedBody = AesDecrypt(aes, ciphertext);
            context.Request.Body = new MemoryStream(decryptedBody);
        }
    }

    private static Aes CreateAesCbc(string key, string iv)
    {
    ...
    private static byte[] AesDecrypt(Aes aes, string text)
    {
